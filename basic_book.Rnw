...
\begin{itemize}
\item {\bf \Large Archivo:} En esta sección se podrá tratar todo lo relacionado con el manejo de archivos y la salida del programa. Los elementos que lo componen son los siguientes:
\begin{itemize}
\item {\bf Interpretar código fuente R:} Al ser R un lenguaje de programación derivado del S nos permite realizar \texttt{scripts}\footnote{Desde el punto de vista informático un {\bf script} es un archivo de órdenes o procesamiento que por lo regular se almacena en un archivo de texto plano.} desde un editor externo, con esta opción podemos ejecutar dicho código en la consola siempre y cuando, la extensión del archivo sea del tipo \texttt{.R}.
\item {\bf Nuevo script:} Al ejecutarse se abrirá un editor de lenguaje R para crear scripts, el mismo que puede ser llamado desde la consola.
\item {\bf Abrir script:} Mediante esta opción, los scripts que han sido generados u obtenidos de alguna manera pueden ser visualizados para posteriormente editarlos.
\item {\bf Mostrar archivo(s):} Sirve para abrir, visualizar o editar cualquier script, la principal diferencia con la opción Abrir script es que éste no puede editarlos directamente, simplemente visualizarlos. Se pueden abrir varios archivos a la vez.
