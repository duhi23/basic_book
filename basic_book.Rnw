...
\subsection{Modificación de vectores}
...
Para eliminar los elementos de un vector usaremos el signo '-' adelante de la posición, como se muestra en el siguiente ejemplo:
<<>>=
...
Recordemos que R es un lenguaje funcional, por lo que cualquier operador, incluido '+' es una función. Veamos un ejemplo:
<<>>=
# sumamos 2 elementos 
3 + 5
# otra alternativa
...
El ejemplo anterior muestra que el producto entre vectores de igual longitud se da entre elementos. En el caso que se multiplique un vector por un escalar el resultado es el siguiente:
<<>>=
# Producto
2 * c(1, 3, 5, 7)
c(3, 4) * c(1, 3, 5, 7)
