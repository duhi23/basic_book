...
En las secciones anteriores mencionamos que R es un lenguaje de programación orientada a objetos, por lo que cualquier cosa que exista en R (variables, datos, funciones, etc.) es un objeto. Para conocer los objetos que se tienen en el espacio de trabajo utilizaremos el siguiente comando:
<<>>=
ls()
@
