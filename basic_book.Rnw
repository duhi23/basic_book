...
\subsubsection{Respaldando información}
...
El primero de ellos permite almacenar en disco los objetos que desee el usuario (almacenamiento parcial), dicho comando puede ser configuraado de tal manera que almacene todos los objetos que se encuentra válidos en el área de trabajo.
<<eval=FALSE>>=
# si deseamos guardar el objeto "datos_banco" con el nombre "base" 
save(datos_banco, file = "base.RData")
# para el caso que se desee almacenar todo con el nombre "info"
...
\begin{enumerate}
   ...
   \item 
\end{enumerate}
La principal ventaja que posee R Analytic Flow sobre el resto de interfaces gráficas es su ventana de diseño de flujo sobre la cual se pueden colocar diversos nodos, los cuales simplifican de sobremanera la complejidad de procesos o script grandes. Esto ha hecho que R Analytic Flow vaya adquiriendo mayor cantidad de usuarios día tras día. 
...
\begin{enumerate}
   ...
   \item El código fuente se encuentra administrado únicamente por Ef-prime, Inc.
\end{enumerate}
