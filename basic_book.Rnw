...
El proyecto R se inició como un experimento en tratar de utilizar métodos de Lisp en la construcción de un pequeño banco de pruebas que sirvan para evaluar posibles construcciones de entornos estadísticos. Desde el inicio del proyecto se decidió usar la syntaxis de S, la misma que con el transcurso del tiempo se volvió irresistible.\newline

El proyecto R se inicia con Ross Ihaka tras haber obtenido acceso a cierta información importante sobre el lenguaje S, misma que fue publicada por John Chambers y Rick Becker. En un corto tiempo, Ross nota las similitudes existentes entre S y Scheme, y en su afán de mostrarse ante Alan Zaslavsky le propone indicar el uso del ámbito léxico para la obtención de variables propias. Sin ninguna copia de Scheme trata de mostrarle usando S, sin embargo sus intentos fracasan debido a las diferencias entre las reglas de asignación que tienen S y Scheme, esto le llevó a darse cuenta que S requería ciertas funcionalidades extras para lograr convertirse en un entorno estadístico.\newline

Tiempo más tarde, Robert Gentleman $\&$ Ross Ihaka llegan a ser colegas en la Universidad de Auckland\footnote{Universidad pública situada en Auckland. Fue fundada en 1883 como parte de la Universidad de Nueva Zelanda.} ambos mostrando interés en temas de Estadística Computacional, como profesores del Departamento de Estadística ven la necesidad de mejorar un ambiente del laboratorio de computación e inician el desarrollo de su interpretador. Su primer interpretador consistía de 1000 líneas de código en C \newline 
...
\begin{enumerate}
   ...
   \item Facil implementación de tareas en cada nodo.
...
En las secciones anteriores mencionamos que R es un lenguaje de programación orientado a objetos, por lo que cualquier cosa que exista en R (variables, datos, funciones, etc.) es un objeto. Para conocer los nombres de los objetos presentes en el área de trabajo utilizamos el comando \code{\texttt{ls()}}.\newline

Iniciemos creando los vectores xvec y yvec para evidenciar que dichos objetos aparecen en nuestra área de trabajo una vez creados.
<<>>=
...
En el ejemplo anterior se utilizó el operador de \emph{asignación estándar} "$\leftarrow$", el cual asigna un valor a una variable, en algunas ocasiones los usuarios principiantes de R utilizan el símbolo "$=$", sin embargo, esto no es recomendable dado que en algunas situaciones podemos tener conflictos de interpretabilidad.\newline
...
Dentro de la coerción un evento importante se produce cuando un vector logical coerciona a un vector integer o double, pues en estos casos el TRUE se convierte en 1 y el FALSE en 0. Esto es muy útil en conjunción con las funciones \code{\texttt{sum()}} y \code{\texttt{mean()}}.
<<>>=
