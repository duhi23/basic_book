...
Los nombres que se les asigna a los objetos en R pueden ser de cualquier longitud y éstos pueden combinar letras, números y caracteres especiales tales como: "_", ",", "." la única exigencia al momento de asignar un nombre a un objeto es que el mismo inicie con una letra. Aunque se recomienda en lo posible no hacer uso del caracter "." debido que el mismo tiene una funcionalidad más avanzada que será detallada más adelante.  
